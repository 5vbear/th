...
function zTreeOnClick(event, treeId, treeNode) {
	...
	window.document.form1.sel_nodes_type.value = treeNode.mactype;
};
...
function cleanProgramlist() {
	...
	var advertType = document.getElementById("advert_type").value;
	var selNodesType = window.document.form1.sel_nodes_type.value;
	var url = "<%= url %>"+"?pageId=jsp_programlist_send_id&funcId=func_programlist_clean_id&advert_type="+advertType+"&sel_nodes_info="+v+"&sel_nodes_type="+selNodesType;
...
<div class="search_title"><span>广告管理-节目单发布</span></div>
<div id="body" style="height:580; width:250; float: left;overflow:scroll;overflow-x:hidden"  class="sdmenu" > 
	...
	<table>
		<tr>
  			<td>端机类型：</td>
  			<td>
  				<input type="hidden"" id="machingType" name="machingType"/>
  				<%
  				if(osTypes != null && osTypes.length > 0){
					for(int i=0; i<osTypes.length; i++){
						out.print("<input type='checkbox' name='osType' value='" + osTypes[i].get("OS") + "' checked='checked'/>" + osTypes[i].get("OS"));
						out.print("&nbsp&nbsp");
						if ( i>0 && (i+1)%4 == 0) {
							out.print("<br/>");
						}
		...
		<tr>
  			<td><input type="button" value="下发" onclick="send();"/></td>
  		</tr>
	</table>
	...
	<input type="hidden" name="sel_nodes_type" value=""/>
</form>
</body>
</html>