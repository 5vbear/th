...
<%
	...
	String url = strContextPath + "/backup.html";
...
%>
<%
	HashMap map = (HashMap)request.getAttribute( "backup_info" );
...
<SCRIPT LANGUAGE="JavaScript" type="text/javascript">
	$(document).ready(function(){
		...
		$('#newone').click(function(){
			...
			if(text =='制订数据库备份计划'){
				flg = true;
				$(this).val('取消');
				document.getElementById("backup_internal1").disabled = false;
				document.getElementById("clear_internal1").disabled = false;
				document.getElementById("savenew").disabled = false;
			}else{
				$('#backup_internal1').val('0');
				$('#clear_internal1').val('0');
				$(this).val('制订数据库备份计划');
				document.getElementById("backup_internal1").disabled = true;
				document.getElementById("clear_internal1").disabled = true;
				document.getElementById("savenew").disabled = true;
			}
		});
		$('#savenew').click(function(){
			$('#deal').val('backup_update');
			var internalVal = $('#backup_internal1').val();
			var clear_internalVal = $('#clear_internal1').val();
			if(internalVal =='0'||clear_internalVal=='0'){
				alert('请选择"备份周期"和"备份清除周期"!');
				return;
			}
			if(clear_internalVal=='300'&&internalVal!='60'){
				alert("备份间隔时间不能大于备份清除时间,请重新选择!");
				return;
			}
			if(internalVal !='60'){
				if(parseInt(internalVal)>parseInt(clear_internalVal)){
					alert("备份间隔时间不能大于备份清除时间,请重新选择!");
					return;
				}
			}
			var backup = transition(internalVal);
			var clear = transition(clear_internalVal);
			$('#renew_internal').val(internalVal);
			$('#renew_description').val(backup);
			$('#renew_clear_internal').val(clear_internalVal);
			$('#renew_clear_description').val(clear);
			$('form').submit();
		});
		$('#updateone').click(function(){
			...
			if(text =='修改数据库备份计划'){
				$(this).val('取消');
				$('#backup_internal2').css('display','block');
				$('#now_internal').css('display','none');
				$('#clear_internal2').css('display','block');
				$('#now_clear_internal').css('display','none');
				$('#message2').css('display','block');
				document.getElementById("saveupdate").disabled = false;
			}else{
				$(this).val('修改数据库备份计划');
				$('#backup_internal2').val('0');
				$('#clear_internal2').val('0');
				$('#backup_internal2').css('display','none');
				$('#now_internal').css('display','block');
				$('#clear_internal2').css('display','none');
				$('#now_clear_internal').css('display','block');
		...
		$('#saveupdate').click(function(){
			$('#deal').val('backup_update');
			var internalVal = $('#backup_internal2').val();
			var clear_internalVal = $('#clear_internal2').val();
			if(internalVal =='0'||clear_internalVal=='0'){
				alert('请选择"备份周期"和"备份清除周期"!');
				return;
			}
			...
			$('#renew_clear_internal').val(clear_internalVal);
			$('#renew_clear_description').val(clear);
			if(confirm('确定将数据库备份周期修改为"'+backup+'",'+'清理周期修改为"'+clear+'"吗?')){
				$('form').submit();
			}
		});
		$('#clear').click(function(){
			if(confirm("确定清理历史备份文件吗?")){
				$('#deal').val('backup_clear');
				$('form').submit();
			}
		});
	...
	function setServer(){
		...
		var fwTimeObj = document.getElementById("dzfwTime").value;
		var dbTimeObj = document.getElementById("dzdbTime").value;
		var url = "<%= Define.MONITOR_RIGHT_SETTTING_SERVLET %>?fwTime=" + fwTimeObj + "&dbTime=" + dbTimeObj + "&funcId=8";
		...
		function setServerName(){
			...
			var fwNameObj = document.getElementById("dzfwName").value;
			var url = "<%= Define.MONITOR_RIGHT_SETTTING_SERVLET %>?fwName=" + fwNameObj + "&funcId=9";

			xmlHttp.open("GET", url, false);
			xmlHttp.onreadystatechange = this.callBackName;
			xmlHttp.send(null);
			}
...
<body>
	...
	<div class="x-title"><span>&nbsp;&nbsp;系统管理-数据库备份/服务订制/清除计划/</span></div>
	<div></div>
	<form class="x-client-form" method="POST" name="form_data" action="<%=url %>">
		<input type="hidden" name="deal" id="deal" value=""/>
		<input type="hidden" name="renew_internal" id="renew_internal" value=""/>
		<input type="hidden" name="renew_clear_internal" id="renew_clear_internal" value=""/>
		<input type="hidden" name="renew_description" id="renew_description" value=""/>
		<input type="hidden" name="renew_clear_description" id="renew_clear_description" value=""/>
		<div>
		
		<%if(null ==map||map.size()==0){%>
		<div>
		<div id="message_noplan"><h3>现在没有数据库备份计划</h3></div>
		<table class="x-data-table">
			<tr id="redo2">
				<th class="x-data-table-th" width="20%">请选择备份周期:</th>
				<td class="x-data-table-td">
					<select id="backup_internal1" style="width: 100px;" name="backup_internal1" disabled="disabled">
						<option id="no_backup" value="0" >无</option>
						<option id="min_backup" value="60" >1分钟</option>
						<option id="day_backup" value="1" >每天</option>
						<option id="week_backup" value="7">7天</option>
						<option id="month_backup" value="30">30天</option>
					</select>
				</td>
			</tr>
			<tr id="redo2_clear">
				<th class="x-data-table-th" width="20%">请选择备份清除周期:</th>
				<td class="x-data-table-td">
					<select id="clear_internal1" style="width: 100px;" name="clear_internal1" disabled="disabled">
						<option id="no_clear" value="0" >无</option>
						<option id="five_min_clear" value="300" >5分钟</option>
						<option id="week_clear" value="7" >7天</option>
						<option id="month_clear" value="30">30天</option>
						<option id="half_year_clear" value="180">180天</option>
					</select>
				</td>
			</tr>
		</table>
		...
		<div  class="x-client-form" style="white-space:nowrap;margin-top: 3px;padding-top: 3px;">
			<input class="tableBtn" type="button" name="savenew" disabled="disabled" id="savenew" readonly="readonly" value="保存"  /> 
			<input class="tableBtn" type="button" style="width: 140px;" name="newone" id="newone" value="制订数据库备份计划" />
		</div>
		<%}else{ %>
			<%
				String internal = (String)map.get("BACKUP_INTERNAL");
				String clear_internal = (String)map.get("CLEAR_INTERNAL");
				String status = (String)map.get("STATUS");
				String time = (String)map.get("OPERATETIME");
				String name = (String)map.get("NAME");
				String description = (String)map.get("DESCRIPTION");
				String clear_description = (String)map.get("CLEAR_DESCRIPTION");
				
			%>
		<div>
		<table class="x-data-table">
			<tr>
				<th style="width: 20%" class="x-data-table-th">数据库备份周期:</th>
				<td class="x-data-table-td">
				<input type="text"  name="now_internal" size="20" id="now_internal" disabled="disabled" value="<%=description%>" />
				<select id="backup_internal2" style="width: 136px;" name="backup_internal2">
						<option id="no_backup" value="0" >--请选择--</option>
						<option id="min_backup" value="60" >1分钟</option>
						<option id="day_backup" value="1" >每天</option>
						<option id="week_backup" value="7">7天</option>
						<option id="month_backup" value="30">30天</option>
			...
			<tr>
				<th style="width: 20%" class="x-data-table-th">数据库备份清除周期:</th>
				<td class="x-data-table-td">
				<input type="text"  name="now_clear_internal" size="20" id="now_clear_internal" disabled="disabled" value="<%=clear_description%>" />
				<select id="clear_internal2" style="width: 136px;"  name="clear_backup_internal2">
						<option id="no_clear" value="0" >--请选择--</option>
						<option id="five_min_clear" value="300" >5分钟</option>
						<option id="week_clear" value="7" >7天</option>
						<option id="month_clear" value="30">30天</option>
						<option id="half_year_clear" value="180">180天</option>
			...
			<tr>
				<th style="width: 20%" class="x-data-table-th">备份计划制订人:</th>
				<td class="x-data-table-td"><input type="text" size="20" name="operator" id="operator" disabled="disabled" value="<%=name%>" /></td>
			</tr>
			<tr>
				<th style="width: 20%" class="x-data-table-th">备份计划制订时间:</th>
				<td class="x-data-table-td"><input type="text" size="20" name="operatetime" id="operatetime" disabled="disabled" value="<%=time%>" /></td>
			</tr>
		</table>
		</div>
		<div class="x-client-form" style="white-space:nowrap;">
    		<input class="tableBtn" type="button" name="saveupdate" disabled="disabled" id="saveupdate" value="保存"  /> 
			<input class="tableBtn" type="button" name="updateone" style="width: 140px;" id="updateone" value="修改数据库备份计划" /> 
  		</div>
		<%} %>
		<div>
		<table class="x-data-table">
			<tr>
				<th style="width: 20%" class="x-data-table-th">订制服务时间:</th>
				<td class="x-data-table-td"><input type="text" size="20" id="dzfwTime" name="dzfwTime" value="" onfocus="setday(this)"/> </td>
			</tr>
			<tr>
				<th style="width: 20%" class="x-data-table-th">订制DB时间:</th>
				<td class="x-data-table-td"><input type="text" size="20" id="dzdbTime" name="dzdbTime" value="" onfocus="setday(this)"/></td>
			</tr>
		</table>
		</div>
		<div class="x-client-form" style="white-space:nowrap;">
    		<input class="tableBtn" type="button" name="setService" id="setService" value="设置时间" onclick="setServer()" /> 
  		</div>
  		<div>
		<table class="x-data-table">
			
			<tr>
				<th style="width: 20%" class="x-data-table-th">订制服务名称:</th>
				<td class="x-data-table-td"><input type="text" size="20" name="dzfwName" id="dzfwName" value="" /></td>
			</tr>
		</table>
		</div>
		<div class="x-client-form" style="white-space:nowrap;">
    		<input class="tableBtn" type="button" name="setServiceName" id="setServiceName" value="设置服务名称" onclick="setServerName()" /> 
  		</div>
		</div>
	</form>
