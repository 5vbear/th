	...
	function btnOperations() {
		...
		this.search = function(){
			...
			//}else{
				...
				window.document.form_data.action = "/th/jsp/updateManagement/UpdateHistorySearch.html";
				// 拼接搜索条件 
				var serCond = "";
...
<body>
	...
	<div class="x-title"><span>&nbsp;&nbsp;升级管理-升级历史查询</span></div>
	...
	<form class="x-client-form" method="POST" name="form_data" action="">
		...
		<div id="foldId" style="width: 100%; display: block; height: 30px; line-height: 30px; background-color: #B2DFEE">
			<div style="float: left">
				<span>&nbsp;&nbsp;机器名</span> 
				<input type="text" name="macMark" id="macMark" value="<%=ConMacMark%>" />
			</div>
			<div style="float: left">
				<span>&nbsp;&nbsp;升级包</span> 
			...
			<div style="float: left">
				<span>&nbsp;&nbsp;时间范围</span>
				<input type="text" size="10" id="searStartTime" name="searStartTime" value="<%=ConTimeStart%>" onclick="getDateString(this,oCalendarChs)"/> 
				<span>&nbsp;---&nbsp;</span>
				<input type="text" size="10" id="searEndTime" name="searEndTime" value="<%=ConTimeEnd%>" onclick="getDateString(this,oCalendarChs)"/>				
			</div>
			<!-- 王经纬   add  start -->
...
//								+ pagesNum + "页][排序方式:记录ID/排序类型:升序][每页" + pageLimit + "条][共" + listCount + "条]</td></tr>" );
						...
						out.print( "<tr><th style='width: 2%' class='x-data-table-th'><input type='checkbox' name='upHistoryListCheckbox' value='-10' disabled='true' style='visibility: hidden'/></th>"
								+ "<th style='width: 8%' class='x-data-table-th'>机器名</th>"
								+ "<th style='width: 14%' class='x-data-table-th'>升级包</th>"
								+ "<th style='width: 18%' class='x-data-table-th'>说明</th>"
