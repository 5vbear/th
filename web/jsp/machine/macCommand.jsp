...
<html>
	<head>
		...
		<script language="JavaScript">
			...
			var zTree;
			$(function() {
				zTreeNodes = <%=(String)request.getAttribute("orgMacsList")%> ;
			}); 
			...
			function treeSelected(event, treeId, treeNode){
				if(treeNode.mactype == 'mac'){
					sub.location.href = '/th/machineServlet?model=command&method=macOperView&macIDStd='+treeNode.id
				}else if(treeNode.mactype == 'org'){
					sub.location.href = '/th/machineServlet?model=command&method=orgOperView&orgIDStd='+treeNode.id
				}else if(treeNode.mactype == 'type'){
					var parentNode=treeNode.getParentNode();
					sub.location.href = '/th/machineServlet?model=command&method=orgOperTypeView&orgTypeIDStd='+treeNode.id+"&pName="+parentNode.name+"&tName="+treeNode.name
				}
			}	
			function forwardRemote(id){
				$('form').attr('action','/th/remote.html?deal=remote_top');
				var i1 = window.frames['sub'];  
				//var iframe =window.frames[0];也可以  
				//获取iframe中的元素值  
				var valu=i1.document.getElementById("macIDStd").value
				$('#machineID').val(valu);
				$('form').submit();
			}
			function forwardDeleteApp(id){
				$('form').attr('action','/th/deleteApp.html?deal=deleteApp_top');
				var i1 = window.frames['sub'];  
				//var iframe =window.frames[0];也可以  
				//获取iframe中的元素值  
				var valu=i1.document.getElementById("macIDStd").value
	...
	<body style="background-color: #fff;">
		...
		<input type="hidden" name="lastPage" id="lastPage" value="non_monitor"/>
		<div class="x-title"><span>&nbsp;&nbsp;端机管理-端机指令</span></div>
		<table><tr style ="height:3px"></tr></table>
		<div style="height: 520px; padding-top: 0px; float: left;">
    		<ul id="menu" class="ztree"></ul>
		</div>
		<div>
			<Iframe src="/th/jsp/machine/macCommandView.jsp" scrolling="no" height="580" width="710" frameborder="0" name="sub" id="sub"></iframe>
		</div>
		</form>
	</body>
</html>