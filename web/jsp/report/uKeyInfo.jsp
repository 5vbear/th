...
<%
	...
	ArrayList<UKeyBean> viewList = (ArrayList<UKeyBean>) request.getAttribute( "viewList" );
	...
	useNum = StringUtils.isBlank( useNum ) ? "" : useNum;
...
<script type="text/javascript">
	...
	function onFold(id){
  	...
  	function doSearch(){
  		if(check()){
  			document.machineOpenRateForm.action = "/th/Report?reportPage=4&functionCode=2";
  	...
  	function exportReport(){
  		if(check()){
	  	document.machineOpenRateForm.action = "/th/Report?reportPage=4&functionCode=3";
	  	document.machineOpenRateForm.submit();
  		}else{
  	  	  	alert('请选择时间');
  	  	}
...
<form class="x-client-form" action="" name="machineOpenRateForm" method="post">
  <div class="x-title"><span>&nbsp;&nbsp;报表管理-网银盾使用统计</span></div>
  ...
  <div id="searchFormId" style="background-color:#B2DFEE">
    <table cellpadding="0" border="0">
      <tr>
        ...
        <td width="33%" style="border:0;text-align:left;">使用次数&nbsp;>=&nbsp;<input name="useNum" type="text" value="<%=useNum %>" size="22" /></td>
      </tr>
      <tr>
  ...
  <div>
    ...
    <div id="summaryDataId" style="<%=summaryDivStyle%>">
      <table class="x-data-table">
        <tr>
          <th colspan ="4" style="width:100%" class="x-data-table-th">网银盾使用统计汇总表</th>
        </tr>             
        <tr>
          <th style="width:30%" class="x-data-table-th">组织名称</th>
          <th style="width:30%" class="x-data-table-th">机器名</th>
          <th style="width:20%" class="x-data-table-th">网银盾使用次数</th>
          <th style="width:20%" class="x-data-table-th">最后一次使用时间</th>
        </tr>
 		<%
 			if ( viewList != null && viewList.size() > 0 ) {
 				for ( int i = 0; i < viewList.size(); i++ ) {
 					UKeyBean uKey = (UKeyBean) viewList.get( i );
		...
		<tr class="<%=trClass%>" onmouseover='OMOver(this);' onmouseout='OMOut(this);'>
			<td class="x-data-table-td"><%=uKey.getOrgName()%></td>
			<td class="x-data-table-td"><%=uKey.getMachineMark()%></td>
			<td class="x-data-table-td"><%=uKey.getUseNum()%></td>
			<td class="x-data-table-td"><%=uKey.getLatestTime()%></td>
		</tr>
		<%
				}
			}
