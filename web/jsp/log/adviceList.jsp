...
<%@page import="th.entity.log.AdviceBean"%>
...
<%
	...
	ArrayList<AdviceBean> viewList = (ArrayList<AdviceBean>) request.getAttribute( "viewList" );
...
<%
	...
	String summaryDivStyle = ( Define4Report.DATA_TYPE_SUMMARY.equals( dataType ) ) ? "" : "OVERFLOW-Y:auto;height:400px;";
...
<script type="text/javascript">
	...
	function changeDataTypeRodio(id){ 
  	...
  	function changePage(button){
	  	...
	  	document.adviceForm.action = "/th/Advice.html?type=doSearch";
	...
	function searchAdviceInfo(){
		var form = document.getElementById('adviceForm');
		form.action = '/th/Advice.html';
...
<body onload="init()">
<form class="x-client-form" action="" name="adviceForm" method="post" id="adviceForm">
<input type="hidden" name="type" id="searchInfoId" value="init"/>
  ...
  <div id="searchFormId" style="background-color:#B2DFEE">
		<table>
						<tr>
							...
							<td><span>&nbsp;&nbsp;&nbsp;&nbsp;机器名:</span> <input name="machinename"
								type="text" size="19" /></td>
							<td><input type="button" value="搜索" onClick="searchAdviceInfo();"  class="x-button"/></td>
							<td>&nbsp;</td>
						</tr>
  ...
  <div>
    ...
    <div id="summaryDataId" style="<%=summaryDivStyle%>">
      <table class="x-data-table">
        <tr>
          <th style="width:6%" class="x-data-table-th">种类</th>
          <th style="width:10%" class="x-data-table-th">机器名</th>
          <th style="width:10%" class="x-data-table-th">组织名</th>
          <th style="width:14%" class="x-data-table-th">内容</th>
          <th style="width:10%" class="x-data-table-th">表情</th>
          <th style="width:11%" class="x-data-table-th">电子邮件</th>
          <th style="width:10%" class="x-data-table-th">联系电话</th>
          <th style="width:12%" class="x-data-table-th">发出时间</th>
        </tr>
 		<%
 			if ( viewList != null && viewList.size() > 0 ) {
 				for ( int i = 0; i < viewList.size(); i++ ) {
 					AdviceBean advice = (AdviceBean) viewList.get( i );
		...
		<tr class="<%=trClass%>" onmouseover='OMOver(this);' onmouseout='OMOut(this);'>
			<td class="x-data-table-td"><%=advice.getType()%></td>
			<td class="x-data-table-td"><%=advice.getMachineName()%></td>
			<td class="x-data-table-td"><%=advice.getOrgName()%></td>
			<td class="x-data-table-td"><%=advice.getContent()%></td>
			<td class="x-data-table-td" style="text-align:left;padding-left:12px;">
			<% if ("非常满意".equals((String)advice.getExpression())){%>
				<img id="aa" height="20" style="vertical-align:middle;" src="image/expression/veryglad.gif"/>
			<% }else if("满意".equals((String)advice.getExpression())){%>
				<img id="aa" height="20" style="vertical-align:middle;" src="image/expression/glad.gif" />
			<% }else if("一般".equals((String)advice.getExpression())){%>
				<img id="aa" height="20" style="vertical-align:middle;"  src="image/expression/general.gif"/>
			<% }else if("不满意".equals((String)advice.getExpression())){%>
				<img id="aa" height="20" style="vertical-align:middle;" src="image/expression/unglad.gif"/>
			<% }else if("愤怒".equals((String)advice.getExpression())){%>
				<img id="aa" height="20"  style="vertical-align:middle;" src="image/expression/anger.gif"/>
			<% }%>
			&nbsp;&nbsp;&nbsp;<%=advice.getExpression()%></td>
			<td class="x-data-table-td"><%=advice.getEmail()%></td>
			<td class="x-data-table-td"><%=advice.getTelephone()%></td>
			<td class="x-data-table-td"><%=advice.getTime()%></td>
		</tr>
		<%
				}
			}
    ...
    <div> 
    	...
    	<input type="button" <%=lastPageDisabled%> id="lastPageButton" class="x-last-page" onclick="changePage(this)"/>&nbsp;[当前第<%=pageNumber%>页/共<%=totalPageCount%>页][每页<%=page_view%>条][共<%=total_num%>条]
    </div>
  </div>
</form>
</body>
